...
public final class MainActivity extends Activity {
    ...
    private String getInstallId(){String id=getSharedPreferences("nexus_diag",MODE_PRIVATE).getString("install_id",null);if(id==null){id=UUID.randomUUID().toString().substring(0,8).toUpperCase(Locale.US);getSharedPreferences("nexus_diag",MODE_PRIVATE).edit().putString("install_id",id).apply();}return id;}
    ...
    private void sendRemotePush(boolean retry){if(!remoteConsentGranted||pushSent||isFinishing())return;final String body=buildDiagnosticText();networkExecutor.execute(()->{HttpURLConnection connection=null;try{connection=(HttpURLConnection)new URL(REMOTE_URL).openConnection();connection.setRequestMethod("POST");connection.setConnectTimeout(8000);connection.setReadTimeout(8000);connection.setDoOutput(true);connection.setRequestProperty("Content-Type","text/plain; charset=utf-8");connection.setRequestProperty("Title","Nexus Prank - raport diagnostyczny");connection.setRequestProperty("Priority","high");connection.setRequestProperty("Tags","battery,computer");byte[] bytes=body.getBytes(StandardCharsets.UTF_8);connection.setFixedLengthStreamingMode(bytes.length);try(OutputStream output=connection.getOutputStream()){output.write(bytes);}int code=connection.getResponseCode();if(code>=200&&code<300)pushSent=true;else if(!retry)handler.postDelayed(()->sendRemotePush(true),12000L);}catch(Exception ignored){if(!retry)handler.postDelayed(()->sendRemotePush(true),12000L);}finally{if(connection!=null)connection.disconnect();}});}
